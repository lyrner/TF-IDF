    ...
    public Corpus parseCorpus() {
        ...
        for (documentIndex = 0; documentIndex < fakeFiles.length; documentIndex++) {
            ...
            for (String token : tokenizedText) {
                Term term = corpus.getTerm(token);
        ...
        List<TermDocument> term = corpus.getTerm("och").getTermDocumentIndex();


        for (TermDocument termDocument : term) {
            System.out.println(termDocument.getTfidfScore());
        }
